...
public class CheepRepository : ICheepRepository
{
    ...
    /// <returns>
    ///     A collection of cheeps authored by the specified authors, ordered by descending timestamp, with optional
    ///     pagination applied.
    /// </returns>
    ...
    {
        var cheep = new RepostCheep { Author = author, TimeStamp = timestamp, Content = originalCheep };
        _dbcontext.Cheeps.Add(cheep);
        _dbcontext.SaveChanges();
        return cheep;
    }
    ...
    /// <exception cref="CheepNotFoundException">
    ///     Thrown when no cheep with the specified ID is found or the cheep is not an
    ///     original cheep.
    /// </exception>
