...
public class TestDbInitializer : IDbInitializer
{
    ...
    public void Seed(UserManager<Author>? userManager = null)
    {
        if (!(_chirpContext.Authors.Any() && _chirpContext.Cheeps.Any()))
        {
            ...
            var a13 = new Author
            {
                AuthorId = 13, DisplayName = "Esser", Email = "jones@itu.dk", Cheeps = new List<Cheep>()
            };
            ...
            if (userManager != null)
            {
                // Create the sample users via ASP.NET Identity so that they can be used to login
                userManager.CreateAsync(a11, "LetM31n!");
                userManager.CreateAsync(a12, "M32Want_Access");
            }
            else
            {
                // If no UserManager is supplied, just add the sample users to DB context directly
                authors.Add(a11);
                authors.Add(a12);
            }
            ...
            var c0 = new OriginalCheep
            {
                CheepId = 658,
                AuthorId = a13.AuthorId,
                Author = a13,
                Text = "This cheep only exists in test data, (not production)",
                TimeStamp = DateTime.Parse("1812-11-04 12:11:12")
            };
            ...
            var c657 = new OriginalCheep
            {
                CheepId = 657,
                AuthorId = a12.AuthorId,
                Author = a12,
            ...
            a13.Cheeps = new List<Cheep> { c0 };

            _chirpContext.Authors.AddRange(authors);
